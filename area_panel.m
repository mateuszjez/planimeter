        ...
        if~isempty(data)
            ...
            if isfield(data(pic),'PolygonData')...
                ...
                if ~isempty(data(pic).TotalArea)...
                        &&~isempty(data(pic).PolygonData)
                    data(pic).PolygonData.PolyArea...
                        = data(pic).PolygonData.RelPolyArea...
                        *data(pic).TotalArea;
                    data(pic).PolygonData.Perimeter = sum(sqrt( ...
                          diff(data(pic).PolygonData.RelXData*data(pic).TotXSize).^2 ...
                        + diff(data(pic).PolygonData.RelYData*data(pic).TotYSize).^2));
                    set(txt_area(1),'String'...
                        ,['Polygon area: '...
                        num2str(data(pic).PolygonData.PolyArea)...
                        ' ' data(pic).Units...
                        ' Perimeter: ' ...
                        num2str(data(pic).PolygonData.Perimeter)...
                        ' ' data(pic).Units(1:(end-1))]);
